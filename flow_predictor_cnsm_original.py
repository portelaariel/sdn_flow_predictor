...
class SeriesState:
    ...
    def ingest(self, byte_count: int, ts: float) -> Optional[Dict[str, Any]]:
        ...
        # --- Detecção (ignora séries de ruído abaixo de MIN_RATE_BPS) ---
        if max(self.rate_bps, self.predicted_bps) < MIN_RATE_BPS:
            self.detector.score(residual)          # alimenta a janela mesmo assim
            return None

        z, is_anom = self.detector.score(residual)
        if not is_anom:
            return None
